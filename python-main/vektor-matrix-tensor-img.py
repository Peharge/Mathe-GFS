...
class TensorRepresentation(Scene):
    def construct(self):
        ...
        # Scalar
        scalar = MathTex(r"s = 5", color=scalar_color).scale(0.5).shift(LEFT * 4.5 + UP * 1.5)
        ...
        matrix = MobjectTable(
            ...
            include_outer_lines=True
        ).scale(0.4).shift(RIGHT * 3 + UP * 1.5)
        ...
        tensor_planes = VGroup(
            MobjectTable(
                [
                    ...
                    [MathTex(r"\vdots", color=tensor_color), MathTex(r"\vdots", color=tensor_color), MathTex(r"\vdots", color=tensor_color), MathTex(r"\vdots", color=tensor_color)],
            ...
            MobjectTable(
                ...
                include_outer_lines=True
            ).scale(0.3).shift(RIGHT * 8.5)
        )

        # Tensor weiter nach unten verschieben und zentrieren
        tensor_planes.shift(DOWN * 2)  # Verschiebt den Tensor weiter nach unten
        tensor_planes.shift(LEFT * 6)  # Verschiebt den Tensor weiter nach unten
        ...
        final_text = Text("Scalar, Vector, Matrix, and Tensor", font_size=24, color=BLACK)
